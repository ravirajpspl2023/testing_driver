...
class ODBPRO(ctypes.Structure):
    ...
    _fields_ = [("dummy", ctypes.c_short * 2),
                ("data", ctypes.c_short),
                ("mdata", ctypes.c_short )]
    
    @property
    def __dict__(self):
        # if self.mdata != CNC.MANE_PROGRAM:
        #     CNC.MANE_PROGRAM = self.mdata
        #     logging.info(f"main_program:{self.mdata}")
        #     CNC.RUNNING_PROGRAM = []
        # if self.data not in CNC.RUNNING_PROGRAM and self.mdata != self.data:
        #     CNC.RUNNING_PROGRAM.append(self.data)
        #     logging.info(f"sub_program:{self.data}")
        return dict((f, getattr(self, f)) for f, _ in self._fields_ if f != "dummy")
    

class PRGDIR(ctypes.Structure):
    _pack_ = 4
    _fields_ = [
        ("prg_data", ctypes.c_char * 256),
     ]
